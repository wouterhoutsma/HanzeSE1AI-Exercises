...
def drawGraph(data):
    ...
    plt.xlabel("Stadspopulatie")
    plt.ylabel("Vervoerderswinst")
    plt.show()
...
def computeCost(X, y, theta):
    ...
    # YOUR CODE HERE
    m = X.shape[0]                      # 1
    predictions = np.dot(X, theta)      # 2
    errors = (predictions - y) ** 2     # 3, 4
    J_val = sum(errors) / (2 * m)       # 5
    return J_val[0]
...
def gradientDescent(X, y, theta, alpha, num_iters):
    ...
    for _ in range(num_iters):
        for i,val in enumerate(theta):
            # YOUR CODE HERE
            predictions = np.dot(X, theta.T) # 1
            errors = predictions - y # 2
            errors = np.dot(errors.T, X) # 3
            theta[i] = theta[i] - (alpha * (sum(errors) / m))
